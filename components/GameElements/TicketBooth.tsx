...
export const TicketBooth: React.FC = () => {
  ...
  const [burnTransactionSuccess, setBurnTransactionSuccess] = useState<boolean>(false);
  const [burnTransactionError, setBurnTransactionError] = useState<boolean>(false);
  const [APITransactionSuccess, setAPITransactionSuccess] = useState<boolean>(false);
  ...
  const changeCounter = (value: number): void => {
    if (count + value < 0) 
      setCount(0);
    else if (count + value > maxTickets) 
      setCount(maxTickets);
    else
  ...
  useEffect(() => {
    ...
    const getMalv1 = async () => {
      try {
        ...
        setMaxTickets(Math.floor(+ethers.utils.formatEther(myMal)/ticketPrice));
  ...
  return (
    <>
      ...
      <>
        ...
        <div className="relative p-10">
            <div className="counter flex flex-col items-center mt-4">
              <div className="buttons flex">
                ...
                <h2 className="text-2xl p-2 text-center border-solid border-2 border-white"><span>{count}</span></h2>

                <SmallButton onClick={() => changeCounter(1)}>+1</SmallButton>
                <SmallButton onClick={() => changeCounter(5)}>+5</SmallButton>
                <SmallButton onClick={() => changeCounter(10)}>+10</SmallButton>
                <SmallButton onClick={() => changeCounter(maxTickets)}>MAX</SmallButton>
              </div>
          </div>
        </div>
        <div>
        ...
        <Popup open={isPopupOpen} setOpen={setIsPopupOpen} title="Getting Maltar Tickets">
          <>
            <p className="text-white">Buying tickets require a blockchain transaction and a call the the backend server. This take some time, so please let the transaction finish before doing anything else. Reloading the page can result in lost MAL.</p>
            <p className="text-white">Please wait for the transaction to finish before closing this popup.</p>
            <p className="text-white">Sending transaction - please approve the request.</p>
            {burnTransaction && ( <p className="text-white">Waiting for transaction {burnTransaction} to finish</p>)}
            ...
            {APITransactionSuccess && (
              <p className="text-white font-xl">Success. Now you can close this window</p>
