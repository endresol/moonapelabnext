...
export function StakingS1S2() {
  ...
  //     () => (
  //       <div>
  //         Transaction started: <a href={transaction.hash}>{transaction.hash}</a>
  //       </div>
  //     ),
  //     { autoClose: 5000, pauseOnHover: true, hideProgressBar: false }
  //   );
  ...
  const handleUnstakeS2Action = async (nfts: number[]) => {
    ...
    const transaction = await moonStakingContractS2.unstake721(
      process.env.NEXT_PUBLIC_MOON_APE_LAB_GENESIS_CONTRACT,
      nfts
    );
    toast.info(`transaction started: <a href="">${transaction.hash}</a>`, {
      autoClose: 5000,
      pauseOnHover: true,
  ...
  const handleStakeS2Action = async (nfts: number[]) => {
    ...
    const numberApes = [];
    nfts.forEach((nft) => {
      numberApes.push(Number(nft));
    });
    console.log("numberApes", numberApes);

    const isApprove = await malgenesisContract.isApprovedForAll(
      address,
      process.env.NEXT_PUBLIC_MOONSTAKING_S2_CONTRACT
    );
    ...
    if (!isApprove) {
      ...
      const transaction = await moonStakingContractS2.stake721(
        process.env.NEXT_PUBLIC_MOON_APE_LAB_GENESIS_CONTRACT,
        numberApes
      );
      toast.info(`transaction started: <a href="">${transaction.hash}</a>`, {
  ...
  const handleUnstakeMutantsAction = async (nfts: number[]) => {
    ...
    const transaction = await moonStakingContractS2.unstake721(
      process.env.NEXT_PUBLIC_BREEDING_CONTRACT,
      nfts
    );
    toast.info(`transaction started: <a href="">${transaction.hash}</a>`, {
      autoClose: 5000,
  ...
  const handleApprovalAction = async () => {
    ...
    const transaction = await malgenesisContract.setApprovalForAll(
      process.env.NEXT_PUBLIC_MOONSTAKING_S2_CONTRACT,
      true
    );
  ...
  useEffect(() => {
    ...
    const getStakerNFTs = async () => {
      try {
        ...
        await fetch(`/api/ape_nft/${address}`)
          .then((response) => response.json())
          .then((data) => {
            let filteredApes = removeDuplicateApes(
              data.nft_ids,
              extractIntegers(s2staking[0])
            );
            filteredApes = removeDuplicateApes(
              filteredApes,
              extractIntegers(s1staking[0])
            );
      ...
      {!isLoading && (
        <>
          <div>
            ...
            <div className="mt-10 mb-10">
              <h2 className="text-3xl font-bold">
                My Staked Moon Apes - Season 2
              </h2>
              <h3 className="text-xl mb-4">Staked after 23rd December 2022</h3>
              <p className="mb-6">
                Select apes and click ”UNSTAKE” in order to unstake selected
                Apes. <br />
                Confirm the transaction through Metamask in order to finalise
                changes.
              </p>
              <NftList
                isApe={true}
                ...
                actions={OGseason2UnstakeActions}
              />
            </div>
            <div className="mt-10 mb-10">
              ...
              <NftList
                nftlist={stakedS2Mutants}
                imagepath="https://storage.moonapelab.io/mutants_images/thumbs"
                actions={MutantsUnstakeActions}
              />
            </div>
            <UnstakePets />
            <br />
            <div className="mt-10 mb-10">
              <h2 className="text-3xl font-bold">
                Available Moon Apes to stake
              </h2>
              <p className="mb-6">
                Select the Moon Apes you want to stake and click the ”STAKE”
                button.
                <br />
                Confirm transaction through Metamask in order to finalise
                changes.
              </p>
              <NftList
                nftlist={malGenesis}
          ...
          <Popup
            ...
            title="Approval needed before staking"
          >
            <>
              <p className="text-white">
                Approval is needed for staking to work. Please click the
                approval button and wait for the transaction to be approved.
              </p>
            </>
          </Popup>
        </>
